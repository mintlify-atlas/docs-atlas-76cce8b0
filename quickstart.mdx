...
stelar.onConnection((client) => {
  console.log(`[${client.protocol}] Connected: ${client.id} from ${client.remoteAddress}`);
...
<Tip>
  Use `mode: 'tcp'` on the client for server-to-server communication. The binary TCP protocol has a 7-byte header (versus up to 14 bytes for WebSocket framing), making it ideal for high-throughput microservice messaging.
